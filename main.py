...
def forward_selection(data_set, num_features, num_lines, is_custom):
    ...
    for i in range(num_features):
        ...
        if feature_to_add_at_this_level >= 0:
            ...
            print("Feature set", feature_to_add_at_this_level,
                  "was best, accuracy is", str(round(accuracy, 2)))
    ...
    if(is_custom):
        ...
        print("Finished! Best set of features to use:", set,
              "with accuracy of", str(round(best_so_far_accuracy, 2)), "%")


def custom_forward_selection(data_set, num_features, num_lines, is_custom):
    print("Beginning search")
    current_set_of_features = []
    best_so_far_accuracy = 0
    local_max_accuracy = 0
    ...
    for i in range(num_features):
        ...
        for k in range(1, num_features + 1):
            if (k not in current_set_of_features):
                ...
                if k >= 0:
                    copy_feature_set.append(k)
                accuracy, num_wrong = custom_one_out_cross_validation(data_set, num_features, num_lines, copy_feature_set, best_so_far_accuracy, num_mistakes)
                #accuracy = rand_one_out()
...
def custom_search(data_set, num_features, num_lines):
    ...
    set1, acc1 = forward_selection(
        copy_feature_set, num_features, num_lines // 2, 1)
    set2, acc2 = forward_selection(
        copy_feature_set2, num_features, num_lines // 2, 1)
    ...
    if(acc1 >= acc2):
        print("Finished! Best set of features to use:", set1,
              "with accuracy of", str(round(acc1, 2)), "%")
    elif(acc2 > acc1):
        print("Finished! Best set of features to use:", set2,
              "with accuracy of", str(round(acc2, 2)), "%")
...
def main():
    ...
    if algorithm_select == '3':
        start = time.time()
        custom_forward_selection(new_data, num_features, num_lines, 0)
        end = time.time()
